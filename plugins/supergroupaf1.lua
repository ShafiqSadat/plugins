...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'سوپرگروه با موفقیت حذف شد!'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'ادد' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'گروه از قبل اضافه شده بود.', ok_cb, false)
			end
			print("گروه با موفقیت "..msg.to.print_name.."("..msg.to.id..") اضافه شد.")
			superadd(msg)
			set_mutes(msg.to.id)
		...
		if matches[1] == 'حذف' and is_admin1(msg) and not matches[2] then
			...
			print("گروه با موفقیت "..msg.to.print_name.."("..msg.to.id..") حذف شد.")
			superrem(msg)
			rem_mutes(msg.to.id)
		end
		...
		if matches[1] == 'لینک جدید' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					...
					send_large_msg(receiver, "لینک گروه ساخته شد.")
		...
		if matches[1] == 'تنظیم لینک' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return 'لطفا لینک جدید گروه را بفرستید.'
		end
		...
		if matches[1] == 'لینک' then
			...
			if not group_link then
				return "ابتدا با دستور لینک جدید برای گروه لینک بسازید.\n\nولی اگر شما سازنده گروه نیستید با دستور تنظیم لینک برای گروه لینک تنظیم کنید."
			end
			return 'لینک گروه:\n'..group_link
		end
